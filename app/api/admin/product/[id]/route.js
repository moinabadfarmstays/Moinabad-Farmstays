...
export async function GET(request, context) {
  try {
    ...
    // Attach upcoming approved bookings so the UI can show blocked dates
    const now = new Date();
    ...
    const productWithBookings = {
      ...product,
      bookings: approvedBookings,
    };

    return NextResponse.json(
      { message: "Product fetched successfully", product: productWithBookings },
      { status: 200 }
    );
  } catch (error) {
